...
pub const BlockEmitter = struct {
    ...
    fn emitForInit(self: *BlockEmitter, fs: *const ast.ForStmt, block: *const cfg_mod.BasicBlock) EmitError!void {
        // Evaluate start expression and store to loop variable.
        // FOR loop variables always use double arithmetic, so promote
        // integer values to double before storing.
        const start_raw = try self.expr_emitter.emitExpression(fs.start);
        const start_type = self.expr_emitter.inferExprType(fs.start);
        const start_val = if (start_type == .integer) blk: {
            const t = try self.builder.newTemp();
            try self.builder.emitConvert(t, "d", "swtof", start_raw);
            break :blk t;
        } else start_raw;
        ...
        // Evaluate end expression and store it for the condition check.
        // Also promote to double if needed.
        const end_raw = try self.expr_emitter.emitExpression(fs.end_expr);
        const end_type = self.expr_emitter.inferExprType(fs.end_expr);
        const end_val = if (end_type == .integer) blk: {
            const t = try self.builder.newTemp();
            try self.builder.emitConvert(t, "d", "swtof", end_raw);
            break :blk t;
        } else end_raw;
    ...
    fn emitForCondition(self: *BlockEmitter, header_idx: u32, true_target: u32, false_target: u32) EmitError!void {
        ...
        // Load stored end value.
        const end_val = try self.builder.newTemp();
        try self.builder.emitLoad(end_val, "d", ctx.end_addr);

        // Determine step direction.
        // If STEP is a compile-time negative literal, use >= comparison.
        // Otherwise, emit a runtime check of step sign.
        if (ctx.step_expr) |step| {
            // Check if step is a negative literal at compile time.
            if (step.data == .number) {
                if (step.data.number.value < 0) {
                    // Negative step: loop while cur >= end
                    const cmp = try self.builder.newTemp();
                    try self.builder.emitCompare(cmp, "d", "ge", cur, end_val);
                    try self.builder.emitBranch(
                        cmp,
                        try blockLabel(self.cfg, true_target, self.allocator),
                        try blockLabel(self.cfg, false_target, self.allocator),
                ...
                // Positive literal step: use <= comparison (fall through below)
            } else {
                // Runtime step expression: emit direction check.
                // step_val = load step; if step > 0, use <=; else use >=
                const step_val = try self.expr_emitter.emitExpression(step);
                const step_positive = try self.builder.newTemp();
                const zero = try self.builder.newTemp();
                try self.builder.emit("    {s} =d copy d_0.0\n", .{zero});
                try self.builder.emitCompare(step_positive, "d", "gt", step_val, zero);

                // Generate labels for the two comparison branches.
                const id = self.builder.nextLabelId();
                const pos_label = try std.fmt.allocPrint(self.allocator, "for_cmp_pos_{d}", .{id});
                const neg_label = try std.fmt.allocPrint(self.allocator, "for_cmp_neg_{d}", .{id});
                const done_label = try std.fmt.allocPrint(self.allocator, "for_cmp_done_{d}", .{id});

                try self.builder.emitBranch(step_positive, pos_label, neg_label);

                // Positive step: cur <= end
                try self.builder.emitLabel(pos_label);
                const cmp_pos = try self.builder.newTemp();
                try self.builder.emitCompare(cmp_pos, "d", "le", cur, end_val);
                try self.builder.emitJump(done_label);

                // Negative step: cur >= end
                try self.builder.emitLabel(neg_label);
                const cmp_neg = try self.builder.newTemp();
                try self.builder.emitCompare(cmp_neg, "d", "ge", cur, end_val);
                try self.builder.emitJump(done_label);

                // Merge with phi
                try self.builder.emitLabel(done_label);
                const cmp = try self.builder.newTemp();
                try self.builder.emit("    {s} =w phi @{s} {s}, @{s} {s}\n", .{ cmp, pos_label, cmp_pos, neg_label, cmp_neg });
                try self.builder.emitBranch(
                    cmp,
                    try blockLabel(self.cfg, true_target, self.allocator),
                    try blockLabel(self.cfg, false_target, self.allocator),
        ...
        // Default: positive step, compare cur <= end
        const cmp = try self.builder.newTemp();
        try self.builder.emitCompare(cmp, "d", "le", cur, end_val);
        try self.builder.emitBranch(
            cmp,
            try blockLabel(self.cfg, true_target, self.allocator),
            try blockLabel(self.cfg, false_target, self.allocator),
    ...
    pub fn emitForIncrement(self: *BlockEmitter, block: *const cfg_mod.BasicBlock) EmitError!void {
        ...
        const step_val = if (ctx.step_expr) |step| blk: {
            const raw = try self.expr_emitter.emitExpression(step);
            const step_type = self.expr_emitter.inferExprType(step);
            // Promote integer step to double for double loop arithmetic.
        ...
        try self.builder.emitBinary(next_val, "d", "add", cur, step_val);
...
pub const CFGCodeGenerator = struct {
    ...
    pub fn generate(self: *CFGCodeGenerator, program: *const ast.Program) ![]const u8 {
        ...
        while (func_it.next()) |entry| {
            ...
            // The semantic analyzer stores names in uppercase, so we must
            // uppercase the CFG builder's key before lookup.
            var upper_buf: [128]u8 = undefined;
            ...
            if (func_sym) |fsym| {
                ...
                const duped_upper = try self.allocator.dupe(u8, upper_name);
